...
static float sdf_sample_gray8_bilinear(const uint8_t *pixels, int gw, int gh, int gstride, float fx, float fy);

/*
 * CSV loader for metrics: char,advance_width,bearing_x,bearing_y,width,height,path
 * Simple, robust parser suitable for ASCII glyph packs used by the project.
 */
...
int sdf_metrics_csv_load(const char *csv_path, sdf_metrics_row_t **out_rows, size_t *out_count) {
	if (!csv_path || !out_rows || !out_count) return -1;
	*out_rows = NULL;
	*out_count = 0;
	...
	while (fgets(line, sizeof(line), f)) {
		...
		if (nf < 7) {
			fprintf(stderr, "sdf_metrics_csv_load: skipping malformed row (got %d fields)\n", nf);
			continue;
		}
		...
		if (n == cap) {
			size_t ncap = cap * 2;
			sdf_metrics_row_t *tmp = (sdf_metrics_row_t*)realloc(rows, ncap * sizeof(*rows));
			if (!tmp) { fprintf(stderr, "sdf_metrics_csv_load: OOM grow\n"); free(r.path); break; }
			/* zero the new part */
...
/* Utility: join font_dir and path if path is relative. Result written to out buffer of size out_sz. */
static void join_path(char *out, size_t out_sz, const char *dir, const char *path) {
    ...
    size_t n = snprintf(out, out_sz, "%s/%s", dir ? dir : ".", path);
    if (n >= out_sz) {
...
sdf_font_t* sdf_font_load(const char *font_dir) {
    ...
    if (!font) { sdf_metrics_rows_free(rows, count); return NULL; }
	...
	for (size_t i = 0; i < count; ++i) {
        ...
        if (ch < 32 || ch > 126) continue; /* ASCII printable range */

        char path[1024];
        join_path(path, sizeof(path), font_dir, r->path);

        uint8_t *pixels = NULL; int w = 0, h = 0, stride = 0;
        ...
        if ((r->width > 0 && w != r->width) || (r->height > 0 && h != r->height)) {
            fprintf(stderr, "sdf_font_load: dim mismatch for '%c' (%u): CSV %dx%d, PNG %dx%d\n", (char)ch, ch, r->width, r->height, w, h);
        }

        /* Create glyph */
        sdf_glyph_t *g = (sdf_glyph_t*)calloc(1, sizeof(sdf_glyph_t));
        if (!g) { free(pixels); continue; }
        int cw = (w > 255) ? 255 : w;
        int chh = (h > 255) ? 255 : h;
        g->width = (uint8_t)cw;
        g->height = (uint8_t)chh;
        g->stride = (uint8_t)((stride > 255) ? 255 : stride);
	...
	float line_top = 0.0f, line_bottom = 0.0f, adv_sum = 0.0f; int adv_cnt = 0;
	for (int c = 32; c <= 126; ++c) {
		sdf_glyph_t *g = font->table[c];
		if (!g) continue;
		if (g->bearing_y > line_top) line_top = g->bearing_y;
		float bottom = (float)g->height - g->bearing_y;
		if (bottom > line_bottom) line_bottom = bottom;
		adv_sum += g->advance_width; adv_cnt++;
	}
	...
	if (fallback_h <= 0) fallback_h = 16;
	if (fallback_h < 8) fallback_h = 8;
	if (fallback_h > 96) fallback_h = 96;
	int fallback_w = fallback_h; /* square */
	int stride = fallback_w; /* gray8 tight */
	...
	if (pix) {
		...
		for (int y = 0; y < fallback_h; ++y) {
			...
			for (int x = 0; x < fallback_w; ++x) {
				...
				float dx2 = fabsf(px) - aix; float dy2 = fabsf(py) - aiy;
		...
		font->box.stride = (uint8_t)((stride > 255) ? 255 : stride);
		font->box.pixels = pix;
		font->box.bearing_x = 0.0f;
		font->box.bearing_y = (line_top > 0.0f) ? line_top : (0.8f * (float)fallback_h);
		font->box.advance_width = (adv_cnt > 0) ? (adv_sum / (float)adv_cnt) : (float)fallback_w;
		font->box_valid = true;
	} else {
		font->box_valid = false;
	}
	...
	/* Attempt to load kerning.csv (optional) */
	{
		...
		sdf_kern_pair_t *pairs = NULL; size_t pcnt = 0;
		if (file_exists(kern_path) && sdf_kerning_csv_load(kern_path, &pairs, &pcnt) == 0 && pcnt > 0) {
	...
	/* Ensure a non-drawing space glyph exists (ASCII 32). If assets omit it, synthesize
	 * a metrics-only glyph so space advances without rendering the fallback box. */
	if (font->table[32] == NULL) {
		float avg_adv = (adv_cnt > 0) ? (adv_sum / (float)adv_cnt) : ((font->line_height > 0.0f) ? 0.33f * font->line_height : 6.0f);
		float em_space = (font->line_height > 0.0f) ? 0.33f * font->line_height : avg_adv;
		float sp_adv = (avg_adv > 0.0f) ? fminf(avg_adv, em_space) : em_space;
		if (sp_adv < 1.0f) sp_adv = (font->line_height > 0.0f) ? 0.25f * font->line_height : 4.0f;
		sdf_glyph_t *sp = (sdf_glyph_t*)calloc(1, sizeof(sdf_glyph_t));
		if (sp) {
			sp->width = 0; sp->height = 0; sp->stride = 0; sp->pixels = NULL;
			sp->bearing_x = 0.0f;
			sp->bearing_y = font->line_top; /* keep consistent baseline metrics */
			sp->advance_width = sp_adv;
			sp->ink_top = 0; sp->ink_bottom = 0;
			font->table[32] = sp;
			font->glyph_count++;
		}
	}
...
int sdf_kerning_csv_load(const char *csv_path, sdf_kern_pair_t **out_pairs, size_t *out_count) {
	if (!csv_path || !out_pairs || !out_count) return -1;
	*out_pairs = NULL; *out_count = 0;
	...
	while (fgets(line, sizeof(line), f)) {
		...
		if (n == cap) {
			size_t ncap = cap * 2;
			sdf_kern_pair_t *tmp = (sdf_kern_pair_t*)realloc(pairs, ncap * sizeof(*pairs));
			if (!tmp) { free(pairs); fclose(f); return -1; }
			pairs = tmp; cap = ncap;
		}
...
float sdf_font_get_kerning(const sdf_font_t *font, unsigned left, unsigned right) {
	if (!font || !font->kern_pairs || font->kern_count == 0) return 0.0f;
	...
	for (size_t i = 0; i < font->kern_count; ++i) {
		...
		if (kp->left == l && kp->right == r) return kp->value;
	}
	return 0.0f;
}
...
static uint8_t* _resample_gray8_scale(const uint8_t *src, int sw, int sh, int sstride, int dw, int dh) {
	...
	for (int y = 0; y < dh; ++y) {
		...
		for (int x = 0; x < dw; ++x) {
			...
			float s = sdf_sample_gray8_bilinear(src, sw, sh, sstride, src_x, src_y);
			float v = s * 255.0f;
			if (v < 0.0f) v = 0.0f; else if (v > 255.0f) v = 255.0f;
			row[x] = (uint8_t)(v + 0.5f);
		}
...
static sdf_glyph_t* _scale_glyph(const sdf_glyph_t *g, float s) {
	if (!g || s <= 0.0f) return NULL;
	int sw = (int)g->width, sh = (int)g->height, ss = (int)g->stride;
	...
	if (dw > 255) dw = 255;
	if (dh > 255) dh = 255;
	uint8_t *pixels = _resample_gray8_scale(g->pixels, sw, sh, ss, dw, dh);
	if (!pixels) return NULL;
	sdf_glyph_t *ng = (sdf_glyph_t*)calloc(1, sizeof(sdf_glyph_t));
	if (!ng) { free(pixels); return NULL; }
	ng->width = (uint8_t)dw;
	ng->height = (uint8_t)dh;
	ng->stride = (uint8_t)dw; /* tight */
	ng->pixels = pixels;
	ng->bearing_x = g->bearing_x * s;
	ng->bearing_y = g->bearing_y * s;
	ng->advance_width = g->advance_width * s;
...
sdf_font_t* sdf_font_create_scaled(const sdf_font_t *base, float target_line_height_px) {
	if (!base || target_line_height_px <= 0.0f || base->line_height <= 0.0f) return NULL;
	float s = target_line_height_px / base->line_height;
	if (s <= 0.0f) s = 1.0f;
	sdf_font_t *f = (sdf_font_t*)calloc(1, sizeof(sdf_font_t));
	if (!f) return NULL;
	for (int i = 0; i < 256; ++i) {
		if (!base->table[i]) { f->table[i] = NULL; continue; }
		f->table[i] = _scale_glyph(base->table[i], s);
		if (f->table[i]) f->glyph_count++;
	}
	if (base->box_valid) {
	...
	if (base->kern_pairs && base->kern_count > 0) {
		f->kern_pairs = (sdf_kern_pair_t*)malloc(base->kern_count * sizeof(sdf_kern_pair_t));
		if (f->kern_pairs) {
...
static inline const sdf_glyph_t* font_glyph(const sdf_font_t *font, unsigned ch);
static float sdf_sample_gray8_bilinear(const uint8_t *pixels, int gw, int gh, int gstride, float fx, float fy);

static void _normalize_dir(float *dx, float *dy) {
	float x = *dx, y = *dy;
	float len = sqrtf(x*x + y*y);
	if (len <= 0.00001f) { *dx = 1.0f; *dy = 0.0f; return; }
	*dx = x / len; *dy = y / len;
}

/* Public: recompute internal caches after attribute changes (kerning, tracking, size, etc.).
 * Ensures per-text scaling is applied and shaping arrays are refreshed. */
...
sdf_text_t* sdf_text_create(sdf_font_t *font, const char *text) {
	...
	if (text && *text) {
		...
		if (t->text) { memcpy(t->text, text, t->text_len + 1); }
	} else {
		t->text_len = 0;
		t->text = NULL;
	}
	...
	t->effects.weight_grow = 0.0f;
	t->effects.shadow_dx = 0.0f;
	t->effects.shadow_dy = 0.0f;
...
void sdf_text_destroy(sdf_text_t *t) {
	...
	if (t->glyph_indices) free(t->glyph_indices);
	if (t->advances) free(t->advances);
	if (t->ofs_x) free(t->ofs_x);
	if (t->ofs_y) free(t->ofs_y);
...
void sdf_text_set_text(sdf_text_t *t, const char *text) {
	if (!t) return;
	if (t->text) { free(t->text); t->text = NULL; t->text_len = 0; }
	if (text && *text) {
		t->text_len = strlen(text);
		t->text = (char*)malloc(t->text_len + 1);
		if (t->text) { memcpy(t->text, text, t->text_len + 1); }
...
void sdf_text_render(sdf_text_t *t, uint8_t *dst, int w, int h, int stride, bool rgba, float time_sec) {
	(void)time_sec;
	/* Update scroll by dt if provided (single-entry convenience) */
	if (t) {
		/* Honor size changes even if the field was written directly without using the setter */
		_sdf_text_apply_size(t);
		if (time_sec != 0.0f && (t->dir_x != 0.0f || t->dir_y != 0.0f) && t->speed != 0.0f) {
			/* Interpret time_sec as dt; callers can pass 0 to skip */
			sdf_text_animate(t, time_sec);
		}
		/* Ensure shaping/placement is up-to-date; rasterization stages 4.2/4.3 will consume this */
		if (t->shape_dirty) {
			_sdf_text_update_shape(t);
		}

		/* If wrapping is enabled, apply wrap in each scrolling direction using canvas bounds */
		if (t->wrap) {
			/* Horizontal wrap across [0, w] when moving along X */
			if (w > 0) {
				if (t->dir_x > 0.0f) {
					sdf_text_wrap_left_to_right(t, 0.0f, (float)w);
				} else if (t->dir_x < 0.0f) {
					sdf_text_wrap_right_to_left(t, 0.0f, (float)w);
				}
			}
			/* Vertical wrap across [0, h] when moving along Y */
			if (h > 0) {
				if (t->dir_y > 0.0f) {
					sdf_text_wrap_top_to_bottom(t, 0.0f, (float)h);
				} else if (t->dir_y < 0.0f) {
					sdf_text_wrap_bottom_to_top(t, 0.0f, (float)h);
				}
			}
		}
	}

	/* ------------- Tasks 4.2 and 4.3: coverage + blending (straight alpha) ------------- */
	/* Debug toggle via env: SDF_TEXT_DEBUG=1 */
	static int s_debug = -1;
	if (s_debug < 0) { const char *e = getenv("SDF_TEXT_DEBUG"); s_debug = (e && *e && e[0] != '0') ? 1 : 0; }

	if (!t || !t->font || !dst || w <= 0 || h <= 0 || stride <= 0) {
		if (s_debug) {
			fprintf(stderr, "[sdf_text] early-exit: t=%p font=%p dst=%p w=%d h=%d stride=%d\n", (void*)t, t ? (void*)t->font : NULL, (void*)dst, w, h, stride);
		}
		return;
	}
	if (t->glyph_count == 0) {
		if (s_debug) {
			fprintf(stderr, "[sdf_text] no glyphs to render. text_len=%zu, wrap=%d, kerning=%d, tracking=%.2f, alpha=%u\n",
					t->text_len, (int)t->wrap, (int)t->kerning_enable, t->tracking, (unsigned)t->alpha);
		}
		return;
	}

	const float softness = 0.12f; /* default smoothstep half-width */
	const float alpha_scale = (float)t->alpha * (1.0f / 255.0f);
	const int channels = rgba ? 4 : 3;

	int pixels_drawn = 0;
	int glyphs_visible = 0;
	for (size_t i = 0; i < t->glyph_count; ++i) {
		unsigned ch = t->glyph_indices[i] & 0xFFu;
		const sdf_glyph_t *g = font_glyph(t->font, ch);
		if (!g || !g->pixels || g->width == 0 || g->height == 0) continue;

		/* Top-left of glyph in canvas (floating), baseline origin is (t->x, t->y) */
		float gx0f = t->x + (float)t->ofs_x[i];
		float gy0f = t->y + (float)t->ofs_y[i];

		/* Compute clipped integer bounds on canvas */
		int x0 = (int)floorf(gx0f);
		int y0 = (int)floorf(gy0f);
		int x1 = (int)ceilf(gx0f + (float)g->width);
		int y1 = (int)ceilf(gy0f + (float)g->height);
				if (x0 < 0) x0 = 0;
				if (y0 < 0) y0 = 0;
				if (x1 > w) x1 = w;
				if (y1 > h) y1 = h;
	...
	glyphs_visible++;
		...
		for (int iy = y0; iy < y1; ++iy) {
			uint8_t *dst_row = dst + (size_t)iy * (size_t)stride;
			for (int ix = x0; ix < x1; ++ix) {
				/* Local coords inside glyph (continuous), pixel center at +0.5 */
				float fx = ((float)ix - gx0f) + 0.5f;
				float fy = ((float)iy - gy0f) + 0.5f;
				float s = sdf_sample_gray8_bilinear(g->pixels, (int)g->width, (int)g->height, (int)g->stride, fx, fy);
				/* Map to coverage via smoothstep around 0.5 */
				float a = smoothstepf(0.5f - softness, 0.5f + softness, s);
				a *= alpha_scale;
				if (a <= 0.0f) continue; /* skip fully transparent */

				uint8_t *p = dst_row + (size_t)ix * (size_t)channels;
				float dr = (float)p[0];
				float dg = (float)p[1];
				float db = (float)p[2];
				float sr = (float)t->color.r;
				float sg = (float)t->color.g;
				float sb = (float)t->color.b;
				float oma = 1.0f - a;
				float or_ = sr * a + dr * oma;
				float og_ = sg * a + dg * oma;
				float ob_ = sb * a + db * oma;
				p[0] = (uint8_t)(or_ < 0.0f ? 0.0f : (or_ > 255.0f ? 255.0f : or_));
				p[1] = (uint8_t)(og_ < 0.0f ? 0.0f : (og_ > 255.0f ? 255.0f : og_));
				p[2] = (uint8_t)(ob_ < 0.0f ? 0.0f : (ob_ > 255.0f ? 255.0f : ob_));
				if (rgba) {
					float da = (float)p[3] * (1.0f / 255.0f);
					float oa = a + da * (1.0f - a);
					p[3] = (uint8_t)(oa * 255.0f + 0.5f);
				}
				pixels_drawn++;
			}
		}
	...
	if (s_debug) {
		...
		if (s_dbg_frames < 60) {
			fprintf(stderr, "[sdf_text] frame dbg: pos=(%.1f,%.1f) dir=(%.2f,%.2f) speed=%.1f wrap=%d rgba=%d glyphs=%zu visible=%d pixels=%d\n",
					t->x, t->y, t->dir_x, t->dir_y, t->speed, (int)t->wrap, (int)rgba, t->glyph_count, glyphs_visible, pixels_drawn);
			if (glyphs_visible == 0) {
				/* Log first and last glyph bbox estimates */
				size_t i0 = 0, i1 = (t->glyph_count ? t->glyph_count - 1 : 0);
				if (t->glyph_count) {
					const sdf_glyph_t *g0 = font_glyph(t->font, t->glyph_indices[i0] & 0xFFu);
					const sdf_glyph_t *g1 = font_glyph(t->font, t->glyph_indices[i1] & 0xFFu);
					float gx0 = t->x + (float)t->ofs_x[i0];
					float gy0 = t->y + (float)t->ofs_y[i0];
					float gx1 = t->x + (float)t->ofs_x[i1];
					float gy1 = t->y + (float)t->ofs_y[i1];
					fprintf(stderr, "  first bbox=[%.1f,%.1f,%.1f,%.1f] last bbox=[%.1f,%.1f,%.1f,%.1f] canvas=[0..%d,0..%d]\n",
							gx0, gy0, gx0 + (g0 ? g0->width : 0), gy0 + (g0 ? g0->height : 0),
							gx1, gy1, gx1 + (g1 ? g1->width : 0), gy1 + (g1 ? g1->height : 0), w, h);
				}
			}
			s_dbg_frames++;
		}
...
/* Apply per-text font scaling to match t->size_px (interpreted as desired line height).
 * Creates and owns a scaled font derived from base_font. When size_px <= 0, reverts to base_font.
 */
static void _sdf_text_apply_size(sdf_text_t *t) {
	if (!t) return;
	if (!t->base_font) t->base_font = t->font; /* fallback */
	if (t->size_px <= 0.0f) {
	...
	if (fabsf(current - target) < 0.01f) return; /* already at desired size */
...
static inline const sdf_glyph_t* font_glyph(const sdf_font_t *font, unsigned ch) {
	...
	if (ch < 256) g = font->table[ch];
	if (!g && font->box_valid) g = &font->box;
	return g;
}

sdf_line_extents_t sdf_text_compute_line_extents(const sdf_text_t *t) {
	sdf_line_extents_t e = {0,0,0};
	if (!t || !t->font || !t->text || t->text_len == 0) return e;
	...
	for (size_t i = 0; i < t->text_len && p[i] != '\0'; ++i) {
		...
		const sdf_glyph_t *g = font_glyph(t->font, ch);
		if (!g) continue;
		if (g->bearing_y > top) top = g->bearing_y;
 ...
 * and per-glyph integer offsets (dx, dy) relative to a baseline origin at (0,0).
 * Final placement for glyph i is:
 *   x0 = baseline_x + ofs_x[i]
 *   y0 = baseline_y + ofs_y[i]
 * Where ofs_x[i] = pen_x + round(bearing_x), ofs_y[i] = - round(bearing_y)
 * And pen_x accumulates round(advance_width + tracking [+ kerning]).
 */
...
static void _sdf_text_update_shape(sdf_text_t *t) {
	...
	if (!t->font || !t->text || t->text_len == 0) {
		...
		if (t->glyph_indices) { free(t->glyph_indices); t->glyph_indices = NULL; }
		if (t->advances) { free(t->advances); t->advances = NULL; }
		if (t->ofs_x) { free(t->ofs_x); t->ofs_x = NULL; }
		if (t->ofs_y) { free(t->ofs_y); t->ofs_y = NULL; }
		t->shape_dirty = false;
		return;
	}
	...
	if (!glyphs || !advs || !dxs || !dys) {
		free(glyphs); free(advs); free(dxs); free(dys);
	...
	for (size_t i = 0; i < t->text_len && p[i] != '\0'; ++i) {
		unsigned ch = (unsigned char)p[i];
		const sdf_glyph_t *g = font_glyph(t->font, ch);
		if (!g) continue; /* extremely unlikely if no fallback */
		glyphs[n++] = (uint16_t)(ch & 0xFFu);
	}

	/* Pass 2: compute adv[i] including tracking and optional kerning with next glyph */
	for (size_t i = 0; i < n; ++i) {
		...
		float adv_f = g ? (g->advance_width + t->tracking + k) : (t->tracking + k);
		int adv_i = iroundf(adv_f);
		advs[i] = (float)adv_i;
	}

	/* Compute per-string line extents (integers) used by valign placement */
	...
	for (size_t i = 0; i < n; ++i) {
		...
		int ib = gh - iby; if (ib < 0) ib = 0;
		if (iby > line_top_i) line_top_i = iby;
		if (ib > line_bottom_i) line_bottom_i = ib;
	}

	/* Pass 3: compute placement using valign and rounded metrics.
	 * Renderer uses baseline origin (0,0). Horizontal placement unchanged; vertical per mode:
	 *  - BASELINE: ofs_y = -round(bearing_y)
	 *  - TOP:      ofs_y = -line_top
	 *  - BOTTOM:   ofs_y = line_bottom - height
	 */
	int pen_x = 0;
	for (size_t i = 0; i < n; ++i) {
		unsigned ch = glyphs[i];
		...
		if (t->valign == SDF_VALIGN_TOP) {
			/* Align glyph tops to the line's top edge */
			dys[i] = -line_top_i;
		} else if (t->valign == SDF_VALIGN_BOTTOM) {
			/* Align glyph bottoms to the line's bottom edge */
...
/* ------------- Task 3.5: Scrolling update and wrap helpers ------------- */

void sdf_text_animate(sdf_text_t *t, float dt) {
	if (!t) return;
	t->x += t->dir_x * t->speed * dt;
	t->y += t->dir_y * t->speed * dt;
}

/* ------------- Task 4.1: Bilinear sampling of 8-bit SDF glyphs ------------- */

static inline uint8_t _pix_at(const uint8_t *base, int stride, int x, int y) {
	return base[(size_t)y * (size_t)stride + (size_t)x];
}
...
static float sdf_sample_gray8_bilinear(const uint8_t *pixels, int gw, int gh, int gstride, float fx, float fy) {
	if (!pixels || gw <= 0 || gh <= 0) return 0.0f;
	/* Map to texel space (pixel centers at integer coords) */
	float tx = fx - 0.5f;
	float ty = fy - 0.5f;
	int x0 = (int)floorf(tx);
	int y0 = (int)floorf(ty);
	float ax = tx - (float)x0;
	float ay = ty - (float)y0;
	/* Clamp sample indices */
	if (x0 < 0) { x0 = 0; ax = 0.0f; }
	if (y0 < 0) { y0 = 0; ay = 0.0f; }
	int x1 = x0 + 1; if (x1 >= gw) { x1 = gw - 1; ax = 0.0f; }
	int y1 = y0 + 1; if (y1 >= gh) { y1 = gh - 1; ay = 0.0f; }
	uint8_t p00 = _pix_at(pixels, gstride, x0, y0);
	uint8_t p10 = _pix_at(pixels, gstride, x1, y0);
	uint8_t p01 = _pix_at(pixels, gstride, x0, y1);
	uint8_t p11 = _pix_at(pixels, gstride, x1, y1);
	float l0 = (1.0f - ax) * (float)p00 + ax * (float)p10;
	float l1 = (1.0f - ax) * (float)p01 + ax * (float)p11;
	float v = (1.0f - ay) * l0 + ay * l1;
	return v * (1.0f / 255.0f);
}

/* Note: The actual rasterization (coverage mapping 4.2 and blending 4.3) will be implemented next.
 * The render entry point above remains the single place to call from the animation loop. */

/* Internal: ensure shaped and compute total horizontal advance width in pixels. */
static int _sdf_text_total_advance_px(sdf_text_t *t) {
	if (!t) return 0;
	if (t->shape_dirty) _sdf_text_update_shape(t);
	int total = 0;
	for (size_t i = 0; i < t->glyph_count; ++i) total += (int)t->advances[i];
	return total;
}

void sdf_text_wrap_left_to_right(sdf_text_t *t, float left_x, float right_x) {
	if (!t) return;
	int wpx = _sdf_text_total_advance_px(t);
	if (wpx <= 0) return;
	/* When the left edge of text moves past the right bound, reset to just left of left bound */
	if (t->x > right_x) {
		t->x = left_x - (float)wpx;
	}
}
...
void sdf_text_wrap_right_to_left(sdf_text_t *t, float left_x, float right_x) {
	...
	int wpx = _sdf_text_total_advance_px(t);
	if (wpx <= 0) return;
	/* When the right edge of text moves past the left bound, reset to right bound */
	if ((t->x + (float)wpx) < left_x) {
		t->x = right_x;
	}
}
...
void sdf_text_wrap_top_to_bottom(sdf_text_t *t, float top_y, float bottom_y) {
	...
	/* Entire line box is [y - e.top, y + e.bottom). If top > bottom bound, reset above top */
	if ((t->y - e.top) > bottom_y) {
		t->y = top_y - e.bottom;
	}
}
...
void sdf_text_wrap_bottom_to_top(sdf_text_t *t, float top_y, float bottom_y) {
	...
	if (t->shape_dirty) _sdf_text_update_shape(t);
	sdf_line_extents_t e = sdf_text_compute_line_extents(t);
	/* If bottom of text box < top bound, reset below bottom */
	if ((t->y + e.bottom) < top_y) {
		t->y = bottom_y + e.top;
	}
}
